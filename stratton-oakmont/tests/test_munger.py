"""Tests for src/agents/munger.py — Charlie Munger persona agent."""
...
def _make_details():
    ...
    details.total_employees = 40000
    return details


class TestMungerAgent:
    @patch("src.agents.munger.call_llm")
    @patch("src.agents.munger.get_company_details")
    @patch("src.agents.munger.get_financial_metrics")
    def test_llm_called_with_data(self, mock_metrics, mock_details, mock_call_llm):
        from src.agents.munger import _analyze_ticker
        ...
        mock_call_llm.return_value = LLMAnalysisResult(
            ...
            reasoning="Wonderful business with high ROIC at a fair price.",
        )
        ...
        assert result.agent_id == "munger_analyst"
    ...
    def test_outer_agent_structure(self, mock_metrics, mock_details, mock_call_llm):
        ...
        mock_call_llm.return_value = LLMAnalysisResult(
            ...
            reasoning="Quality business with durable moat.",
        )
        ...
        result = munger_agent(state)

        assert "messages" in result
        signals = result["data"]["analyst_signals"]["munger_analyst"]
        assert len(signals) == 1
        assert signals[0]["signal"] == "bullish"


class TestBuildFacts:
    def test_facts_include_quality_metrics(self):
        from src.agents.munger import _build_facts
